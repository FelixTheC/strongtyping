...
def docs_from_typing(func, remove_linebreak, style):
    ...
    if func.__doc__:
        ...
        additional_infos = dict([tuple(filter(lambda x: len(x) > 1, map(str.strip, re.split(r'(\$\d)', info))))
                                 for info in additional_infos if info and info[0] == '$'])
    else:
        func_info = f'Function {func.__name__}\n\n'
        additional_infos = {}
    if style == 'rest':
        return docs_from_typing_reST_format(annotations, additional_infos, func_params, remove_linebreak, func_info)
    elif style == 'numpy':
        return docs_from_typing_numpy_format(annotations, additional_infos, func_params, remove_linebreak, func_info)
